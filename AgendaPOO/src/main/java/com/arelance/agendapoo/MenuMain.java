...
enum MenuMain {
    ...
    IMPRIMIR(1, "Ver agenda") {
        ...
        public void runTasker() {
            VistaAgenda.printSchedule(Schedule.getInstance().getContactos());
            //Schedule.getInstance().printSchedule();
        }
    }, BUSCAR(2, "Buscar Contacto") {
        @Override
        public void runTasker() {
            VistaAgenda.mostrarContacto();
        ...
        @Override
        public void runTasker() {
            Schedule.getInstance().addContact();
        }
    }, MODIFICAR(4, "Modificar Contacto") {
        @Override
        public void runTasker() {
            Schedule.getInstance().modifyContact(Schedule.getInstance().findContact(FindMenu.loopFindMenu(), InOut.getData("Escribe el dato que quieres buscar")));
        }
    }, BORRAR(5, "Borrar Contacto") {
        @Override
        public void runTasker() {
            VistaAgenda.borrarContacto();
