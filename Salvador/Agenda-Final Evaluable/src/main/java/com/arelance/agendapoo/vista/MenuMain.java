...
public enum MenuMain {
    ...
    IMPRIMIR(1, "Ver agenda") {
        ...
        public void runTasker() {
            Controlador.getInstance().imprimirAgenda();
        }
    }, BUSCAR(2, "Buscar Contacto") {
        @Override
        public void runTasker() {
            Controlador.getInstance().mostrarContacto(InOut.createContact(3));
//            Controlador.getInstance().mostrarContacto(FindMenu.loopFindMenu(), InOut.getData("¿Que quieres buscar?"));
        }
        ...
        public void runTasker() {
            ...
            Controlador.getInstance().agregarContacto(InOut.createContact(3));
        }
    }, MODIFICAR(4, "Modificar Contacto") {
        @Override
        public void runTasker() {
//            Controlador.getInstance().modificarContacto(FindMenu.loopFindMenu(), InOut.getData("¿A quien quieres modificar?"), InOut.createContact(3));
        }
    }, BORRAR(5, "Borrar Contacto") {
        @Override
        public void runTasker() {
//            Controlador.getInstance().borrarContacto(FindMenu.loopFindMenu(), InOut.getData("¿Que qiueres buscar?"));
        }
    }, BORRARAGENDA(6, "Borrar Agenda") {
        @Override
        public void runTasker() {
            Controlador.getInstance().borrarAgenda();
        }
    }, RESTAURAR(7, "Restaurar copia de seguridad") {
        @Override
        public void runTasker() {
            Controlador.getInstance().rellenarAgenda();
        }
